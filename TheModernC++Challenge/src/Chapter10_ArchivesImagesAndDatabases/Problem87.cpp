    ...
    std::optional<std::string> media_file_description)
{
    ...
    try
    {
        auto media_file{ rtc::movies::MediaFile{ movie_id, media_file_path, media_file_description } };
...
void delete_media(rtc::movies::sqlite_mcpp::database& movies_db, size_t media_id)
{
    try
    {
    ...
    catch (const rtc::movies::sqlite_mcpp::MediaFileIdNotFoundError& ex)
    {
        std::cout << "Error: " << ex.what() << "\n";
    }
}

void list_media(const rtc::movies::sqlite_mcpp::database& movies_db, size_t movie_id)
{
    try
    {
...
void menu(rtc::movies::sqlite_mcpp::database& movies_db)
{
    for (;;)
    {
