...
class ean_13_barcode
{
private:
    static inline auto start_marker{ std::bitset<3>{"101"} };
    static inline auto center_marker{ std::bitset<5>{"01010"} };
    static inline auto end_marker{ std::bitset<3>{"101"} };
...
public:
    explicit ean_13_barcode(const std::string& code_str) : code_str_{ code_str } { encode(); }

    [[nodiscard]] auto get_start_marker() const { return start_marker; }
    [[nodiscard]] auto get_center_marker() const { return center_marker; }
    [[nodiscard]] auto get_end_marker() const { return end_marker; }
    [[nodiscard]] auto get_first_group_bs() const { return first_group_bs_; }
    [[nodiscard]] auto get_second_group_bs() const { return second_group_bs_; }
    ...
    void encode()
    {
        ...
        auto first_digit = digit{ to_uint8_t(code_str_[0]) } ;
        auto first_group_digits = first_group_structure_map[first_digit.get_value()];
        auto second_group_digits = digit_group{ r, r, r, r, r, r };

        first_group_bs_ = first_group_digits.encode(code_str_.substr(1, group_size));
        second_group_bs_ = second_group_digits.encode(code_str_.substr(1 + group_size));
    }

private:
    std::string code_str_{};

    digit_group_bs first_group_bs_{};
    digit_group_bs second_group_bs_{};
};
